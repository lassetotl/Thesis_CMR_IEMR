...
class ComboDataSR_2D:
    def __init__(  # initializes when an instance is created
        ...
        #SiV fysiker PC
        self.data = sio.loadmat(f'C:\\Users\\Radiologi_Fysiker\\Artikkel IEMR Lasse\\Filer 29.01.25\\250129 til lasse\\250129 til lasse\\{filename}')["ComboData_thisonly"]
    ...
    def strain_rate(self, ellipse = 1, plot = 1, save = 1, segment = 0):  
        ...
        if plot == 1:
            ...
            if segment == 0:
                ...
                plt.plot(self.range_TR, cs, c = 'darkblue', lw=2.5, ls='--', label = 'Circumferential')
            ...
            if segment == 1:  # mean angles segments
                ...
                plt.axvline(self.T_es*self.TR*1000, c = 'k', ls = ':', lw = 2) #, label = 'End Systole')
                #plt.axvline(44, c = 'dimgray', ls = '-', lw = 2)
                #plt.axvline(103, c = 'dimgray', ls = '-', lw = 2)
                ...
                for i in self.range_:
                    for sector in range(4):
                        ...
                        plt.scatter([self.range_TR[i]]*len(self.theta2[sector, i]), self.theta2[sector, i], color = color_c, alpha = 0.011*self.n**2)
                        plt.scatter([self.range_TR[i]]*len(self.theta1[sector, i]), self.theta1[sector, i], color = color_s, alpha = 0.011*self.n**2)

                plt.plot(self.range_TR, theta2_mean_global, color_c, lw=2.5, label = 'Mean compression angle')
                plt.plot(self.range_TR, theta1_mean_global, color_s, lw=2.5, label = 'Mean stretch angle')
                
                plt.scatter(np.argmax(theta2_mean_global)*self.TR*1000, np.max(theta2_mean_global), color = color_c, marker = ' ', s = 150, lw = 2)
                plt.scatter(np.argmin(theta2_mean_global)*self.TR*1000, np.min(theta2_mean_global), color = color_c, marker = ' ', s = 150, lw = 2)
                plt.scatter(np.argmax(theta1_mean_global)*self.TR*1000, np.max(theta1_mean_global), color = color_s, marker = ' ', s = 150, lw = 2)
                plt.scatter(np.argmin(theta1_mean_global)*self.TR*1000, np.min(theta1_mean_global), color = color_s, marker = ' ', s = 150, lw = 2)
            ...
            #ra1 = range(len(self.theta1_global_)); ra2 = range(len(self.theta2_global_))
            for i in self.range_:
                std1[i] = np.std(self.theta1_global_[i])
                std2[i] = np.std(self.theta2_global_[i])
            
            print(std1)
            plt.plot(self.range_TR, std1, label = 'stretch')
            plt.plot(self.range_TR, std2, label = 'compression')
            
            plt.xlabel('Time [ms]', fontsize = 15)
            plt.ylabel('SR angle - Standard deviation', fontsize = 15)
            plt.legend(); plt.show()
